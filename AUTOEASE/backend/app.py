...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/autov'
db=SQLAlchemy(app)
...
def download_report():
        ...
        class PDF(FPDF):
            def header(self):
                ...
                self.set_font('helvetica','B',14.0)
                self.set_x(80)
                self.cell(40, 10, 'AUTOVCARD', align='C',border=1,fill=1)
                self.ln(30)

            def footer(self):
                self.set_y(-155)
                self.set_font('helvetica','I',10)
                self.cell(0,10,f'Thank you for using AUTOVACRD... :D',align='L')
                self.cell(0,10,f'Page {self.page_no()}/{pdf.alias_nb_pages()}',align='C')
            ...
            def body(self):
                ...
                h=8
                self.set_fill_color(235, 244, 245)
                self.cell(100,10,f'AUTOCARD',align='C',fill=1,ln=1,border=1)
                self.set_draw_color(0,0,0)
                self.set_fill_color(194, 171, 219)
                ...
                self.cell(w,h,f'card_id : {str(card)}', border=1,ln=True,fill=1)
                self.set_x(60)
                self.cell(w,h,f'Registration id : {str(reg)}', border=1,ln=True,fill=1)
                self.set_x(60)
                self.cell(w,h, f'owner : {str(owner)}', border=1,ln=True,fill=1)
                self.set_x(60)
                self.cell(w,h, f'pollution_id : {str(puc)}', border=1,ln=True,fill=1)
                self.set_x(60)
                self.cell(w,h,f'valid date : {str(pucend)}', border=1,ln=True,fill=1)
                self.set_x(60)
                self.cell(w,h, f'insuranceid : {str(ins)}', border=1,ln=True,fill=1)
                self.set_x(60)
                self.cell(w,h,f'valid date : {str(insend)}', border=1,ln=True,fill=1)
        pdf = PDF('p','mm','Letter')
        pdf.add_page()
        pdf.alias_nb_pages()
        pdf.body()
        ...
        return Response(pdf_file, mimetype='application/pdf', headers={'Content-Disposition':'attachment;filename=employee_report.pdf'})



# if __name__ == "__main__":
#     from waitress import serve
#     serve(app, host="0.0.0.0", port=3066)
app.run(debug = True)
